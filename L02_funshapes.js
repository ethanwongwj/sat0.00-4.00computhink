/*
Task 1: Add the setup() function
*/

function setup() {
   //           w     h
  createCanvas(400, 800);
  background("gray");
}

function draw() {
  fill("orange")
  ellipse(58,100,55,120)
  fill("gold");
  triangle(30, 75, 58, 20, 86, 75);
  fill("gray")
  circle(58,100,20)
  fill("green")
  rect(33,150,50,25)
  fill("orange")
  ellipse(158,100,55,120)
  fill("gold");
  triangle(130, 75, 158, 20, 186, 75);
  fill("gray")
  circle(158,100,20)
  fill("green")
  rect(133,150,50,25)
  fill("orange")
  ellipse(258,100,55,120)
  fill("gold");
  triangle(230, 75, 258, 20, 286, 75);
  fill("gray")
  circle(258,100,20)
  fill("green")
  rect(233,150,50,25)
  fill("orange")
  ellipse(58,250,55,120)
  fill("gold");
  triangle(30,225, 58,170, 86, 225);
  fill("gray")
  circle(58,250,20)
  fill("green")
  rect(33,300,50,25)
  fill("orange")
  ellipse(158,250,55,120)
  fill("gold");
  triangle(130,225, 158, 170, 186, 225);
  fill("gray")
  circle(158,250,20)
  fill("green")
  rect(133,300,50,25)
  fill("orange")
  ellipse(258,250,55,120)
  fill("gold");
  triangle(230, 225, 258, 170, 286, 225);
  fill("gray")
  circle(258,250,20)
  fill("green")
  rect(233,300,50,25)
  fill("orange")
  ellipse(58,400,55,120)
  fill("gold");
  triangle(30, 375, 58, 320, 86, 375);
  fill("gray")
  circle(58,400,20)
  fill("green")
  rect(33,450,50,25)
  fill("orange")
  ellipse(158,400,55,120)
  fill("gold");
  triangle(130, 375, 158, 320, 186, 375);
  fill("gray")
  circle(158,400,20)
  fill("green")
  rect(133,450,50,25)
  fill("orange")
  ellipse(258,400,55,120)
  fill("gold");
  triangle(230, 375, 258, 320, 286, 375);
  fill("gray")
  circle(258,400,20)
  fill("green")
  rect(233,450,50,25)
}


  // Challenge 1: An eye using ellipse() function

  // Challenge 2: A face using circle() function

  // Challenge 3: A house using rect() function

  // Challenge 4: A rocketship using triangle() function

  // Challenge 5: A square face using quad() function

  // Challenge 6: Add some colours to the drawings in the above challenges

  // Challenge 7: Create a funny Pokemon
